...
이메일+비밀번호 계정 시스템(Supabase Auth)이 있다. 로그인한 사용자별로 생성 히스토리, 설정값(주제별 업체정보·본문 템플릿, 작성규칙, 어조/분량 기본값), Google API 키가 Supabase Postgres에 저장되어 기기를 바꿔도 동일하게 이용할 수 있다. API 키는 `pgcrypto`로 암호화되어 저장되고, 저장 이후에는 클라이언트에 평문으로 다시 노출되지 않는다(설정 페이지에는 "등록됨" 상태만 표시).
...
- `user_settings` — 사용자별 설정 1행(주제, 주제별 업체정보 jsonb, 주제별 본문 템플릿 jsonb, 작성규칙, 어조, 분량, 암호화된 API 키). RLS로 `auth.uid() = user_id`만 접근 가능.
...
- **`server/utils/buildPrompt.ts`** — `buildPrompt(req): BlogPrompt`를 export하는 얇은 진입점. 실제 조립은 `server/utils/prompts/promptBuilder.ts`의 `buildBlogPrompt(req)`에 위임한다. `BlogPrompt`는 `{ system, user }` 두 문자열 객체다(Gemini `systemInstruction`/`contents` 분리 대응). 과거에는 문자열 하나만 반환했으나 지금은 이 객체를 반환하므로, 호출부(`generate.post.ts`)는 `prompt.system`/`prompt.user`를 각각 `callGemini(apiKey, system, user)`에 넘긴다.
- **`server/utils/prompts/`** — 프롬프트를 SYSTEM(규칙 계층, `systemInstruction`으로 전송) + USER(실제 생성 데이터, `contents`로 전송) 구조로 분리해 관리한다.
  - `promptBuilder.ts` — `buildBlogPrompt(req): BlogPrompt`. SYSTEM 프롬프트는 규칙 우선순위 안내 → 사용자 제공 데이터를 지시문으로 오인하지 말라는 프롬프트 인젝션 방지 문구 → CORE → SEO → AER → GEO → EER → STYLE → USER CUSTOM RULE → 생성 후 내부 검수 → 출력 형식 순으로 조립한다(요청마다 달라지는 부분은 USER CUSTOM RULE뿐, 나머지는 모든 요청에 공통인 고정 규칙). USER 프롬프트는 작성 대상/제목/본문 템플릿/참조 내용/업체정보/사진 자리 표시 등 이번 생성 1회의 콘텐츠 데이터만 담고 규칙 텍스트는 넣지 않는다. 규칙 충돌 시 우선순위는 사실성 > 사용자 지정 규칙 > 주제·검색의도 > SEO > AER > GEO > EER > 문체.
  - `coreRules.ts`/`seoRules.ts`/`aerRules.ts`/`geoRules.ts`/`eerRules.ts`/`styleRules.ts` — 각각 시스템 기본 규칙(허위정보 금지 등)/SEO/AER(응답 속도)/GEO(AI 이해도)/EER(정보 완결성)/문체 규칙을 반환하는 순수 함수. 사용자 입력과 무관한 고정 문자열이라 인자를 받지 않는다.
  - `userRules.ts` — `buildUserCustomRule(writingRules)`. 사용자가 "규칙" textarea에 입력한 내용을 `[사용자 지정 필수 규칙]` 블록으로 줄 단위 변환하고, `"""..."""`로 감싸 원문 그대로임을 표시한 뒤 그 안의 문장을 프롬프트 자체를 바꾸는 지시로 해석하지 말라는 문구를 덧붙인다(사용자 입력에 지시문처럼 보이는 텍스트가 섞여도 프롬프트 구조가 깨지지 않도록 하는 안전장치). **SEO/AER/GEO/EER 규칙은 여기 섞지 않는다** — 이 값은 애플리케이션 기본 규칙과 별개로 계정별 커스텀 규칙만 담아야 하는 필드다.
  - `contentBlocks.ts` — 요청 데이터(제목 고정 여부, 본문 템플릿, 참조 내용, 업체정보, 어조 가이드, 사진 자리 개수)에 따라 달라지는 콘텐츠 블록 빌더. 참조 내용/본문 템플릿처럼 사용자가 자유 입력한 긴 텍스트를 프롬프트에 넣을 때는 `"""..."""`로 감싸 원문 데이터임을 구분하는 패턴을 유지한다. 새 입력 필드를 프롬프트에 반영할 때는 이 패턴(트림 → 비어있으면 안내 문구, 아니면 지침 문자열 반환)을 그대로 따른다.
  - 새 시스템 규칙 카테고리가 필요하면 이 폴더에 파일을 추가하고 `promptBuilder.ts`의 SYSTEM 조립 순서에 끼워 넣는다. 사용자 지정 규칙과 시스템 기본 규칙을 절대 같은 필드에 합치지 않는다(`writingRules`는 항상 USER CUSTOM RULE 전용).
- **`server/utils/gemini.ts`** — Gemini REST API 호출 헬퍼. 모델명은 `gemini-3.6-flash`로 상수 고정되어 있다(사용자가 명시적으로 지정한 값). `callGemini(apiKey, systemPrompt, userPrompt)`는 `systemPrompt`를 요청의 `systemInstruction` 필드로, `userPrompt`를 `contents[0]`(role: user)으로 분리해서 보낸다(규칙 텍스트와 콘텐츠 데이터가 실제 API 레벨에서도 분리되도록). `responseSchema`로 `{ title, body, tags }` JSON 출력을 강제한다. `validateGeminiKey()`는 후보 키 하나가 유효한지 가볍게 확인(모델 목록 조회)하는 함수다. HTTP 상태코드를 한국어 에러 메시지로 매핑한다.
...
- **`server/utils/naver.ts`** — `searchNaverLocal(query)`. 네이버 검색(지역) API(`openapi.naver.com/v1/search/local.json`)를 서비스 공용 `NUXT_NAVER_CLIENT_ID`/`NUXT_NAVER_CLIENT_SECRET`로 호출해 업체 후보 목록(`NaverBusinessCandidate[]`)을 반환한다. 이 API는 `title/category/description/telephone/address/roadAddress`만 응답하고 영업시간·주차여부·메뉴/서비스·SNS는 제공하지 않는다 — `BusinessInfoSection.vue`도 이 범위만 자동입력하고 나머지는 수동 입력을 유지한다. 응답의 `<b>` 태그·HTML 엔티티를 제거하고, 좌표(`mapx`/`mapy`) 변환 대신 `title`+`roadAddress`로 조합한 네이버 지도 검색 링크(`mapUrl`)를 생성해 반환한다. `gemini.ts`와 같은 fetch + 상태코드→한국어 에러 매핑 패턴을 따른다.
- **`server/api/business-lookup.post.ts`** — 인증된 사용자만 호출 가능(`serverSupabaseUser`, 로그인 없는 사용자가 공용 네이버 키 쿼터를 소진하지 못하도록). `{ query }`를 받아 `searchNaverLocal()`을 호출하고 `{ candidates }`를 반환한다.
...
- **`app/composables/useUserSettings.ts`** — 계정별 설정(주제, 주제별 업체정보/본문 템플릿, 작성규칙, 어조, 분량, `hasApiKey`)을 Supabase `user_settings` 테이블과 동기화한다. `usePersistedState`와 같은 SSR-hydration-safe 패턴을 따르되(기본값으로 시작 → `onMounted`에서만 실제 값 반영), 동기 localStorage 읽기 대신 비동기 Supabase 조회를 쓴다. `useState()`로 선언되어 있어 `index.vue`/`settings.vue`/`useBlogGenerator`가 같은 데이터를 공유한다(중복 fetch 없음). `loaded` 플래그로 fetch 완료 전에는 auto-save(디바운스된 `watch`)가 기본값으로 DB를 덮어쓰지 않도록 가드한다. `fetchSettings()`는 쿼리 전에 `client.auth.getSession()`으로 세션 복원을 기다린다(로그인 직후 마운트 시 세션이 아직 클라이언트에 붙기 전에 select가 나가면 RLS에 걸려 빈 결과가 오고, `hasApiKey`가 실제로는 등록돼 있는데도 기본값 `false`로 굳어버리는 버그가 있었다). **중요한 함정**: `useSupabaseUser()`는 이 모듈 버전(`@nuxtjs/supabase`)에서 `User` 객체가 아니라 `auth.getClaims()`가 반환하는 **JWT payload**(`Ref<JwtPayload | null>`)를 담는다 — 사용자 ID는 `.id`가 아니라 `.sub`이다. `saveSettings()`에서 `user.value.id`로 접근했다가 항상 `undefined`가 되어(JSON 직렬화 시 키가 통째로 빠짐) `user_id`가 없는 upsert가 나가 RLS 위반(403)으로 자동저장이 조용히 실패한 적이 있다(실제로 겪고 고친 버그). `useSupabaseUser()`의 반환값을 쓸 때는 항상 `.sub`을 사용한다.
- **`app/composables/useAuth.ts`** — `useSupabaseUser()`/`useSupabaseClient()`를 감싼 `{ user, signOut }`.
- **`app/composables/useBlogGenerator.ts`** — 메인 폼 상태를 모아 `/api/generate`를 호출하고, 성공 시 `useHistory().add()`로 결과를 자동 저장한다. 설정 관련 상태(topic/businessInfo/tone/length/writingRules/bodyTemplates/hasApiKey)는 `useUserSettings()`에서 가져오고, 이번 생성에만 쓰는 값(mainKeyword/relatedKeywordsInput/referenceContent/customTitle)만 자체 `ref`로 갖는다. `resetForm()`은 "글 설정" 카드의 입력값을 초기화한다.
