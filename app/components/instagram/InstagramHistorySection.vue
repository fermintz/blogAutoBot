...
import type { SavedInstagramCaption } from '~~/shared/types'

const { items, remove, clear } = useInstagramHistory()
...
const selected = ref<SavedInstagramCaption | null>(null)
const isOpen = ref(false)

function view(item: SavedInstagramCaption) {
  selected.value = item
  isOpen.value = true
}
...
<template>
        ...
        >
          ...
          <div class="flex items-center gap-2 mt-1 text-xs text-muted">
            <UBadge
              ...
              size="sm"
            >
              {{ item.result.body.slice(0, 20) }}{{ item.result.body.length > 20 ? '...' : '' }}
            </UBadge>
    ...
    <template #header="{ close }">
      <div class="flex items-center w-full gap-5">
        <div class="flex flex-col gap-0.5 flex-1">
          <span class="font-medium">{{ selected?.storeInfo.name }}</span>
  ...
  <UModal
    ...
    title="전체 삭제"
    description="저장된 인스타 설명글 생성 이력을 모두 삭제합니다. 이 작업은 되돌릴 수 없습니다."
  >
    <template #footer>
